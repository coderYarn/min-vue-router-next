...
/**
 * Checks if two any are equal. This means that both locations are
 * pointing towards the same {@link RouteRecord} and that all `params`, `query`
 * parameters and `hash` are the same
 *
 * @param a - first {@link any}
 * @param b - second {@link any}
 */
export function isSameany(
  stringifyQuery: (query: any) => string,
  a: any,
  b: any
): boolean {
  ...
  return (
    ...
    isSameanyParams(a.params, b.params) &&
 ...
 */
export function isSameRouteRecord(a: any, b: any): boolean {
...
export function isSameanyParams(
  a: any['params'],
  b: any['params']
): boolean {
  ...
  for (const key in a) {
    if (!isSameanyParamsValue(a[key], b[key])) return false
  }

  return true
}

function isSameanyParamsValue(
  a: any | any[],
  b: any | any[]
): boolean {
